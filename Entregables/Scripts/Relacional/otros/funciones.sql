...
--
CREATE
OR REPLACE FUNCTION recomendar_establecimientos(usuario_id INT) RETURNS TABLE(id INT, nombre VARCHAR) AS $ $ BEGIN RETURN QUERY
SELECT
    e.id,
    e.nombre
FROM
...
--
CREATE
OR REPLACE FUNCTION calcular_distancia(usuario_id INT, establecimiento_id INT) RETURNS DOUBLE PRECISION AS $ $ DECLARE distancia DOUBLE PRECISION;
...
--
CREATE
OR REPLACE FUNCTION obtener_promedio_calificacion(est_id INT) RETURNS DECIMAL(3, 2) AS $ $ DECLARE promedio DECIMAL(3, 2);
...
--
CREATE
OR REPLACE FUNCTION contar_visitas_usuario(user_id INT) RETURNS INT AS $ $ DECLARE total_visitas INT;
...
--
CREATE
OR REPLACE FUNCTION obtener_gasto_total_usuario(user_id INT) RETURNS DECIMAL(10, 2) AS $ $ DECLARE total_gasto DECIMAL(10, 2);
