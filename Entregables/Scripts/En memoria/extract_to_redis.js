...
async function main() {
  ...
  try {
    ...
    for (const est of establecimientos.rows) {
      ...
      let minutosCierre =
        parseInt(horasCierre[0]) * 60 + parseInt(horasCierre[1]);
      ...
      redisCommands.write(
        `ZADD establecimientos:por_hora_cierre ${minutosCierre} "${est.id}"\n`
      );
      ...
      // Guardar comando para archivo
      redisCommands.write(
        `HSET establecimiento:${est.id} nombre "${est.nombre}" categoria "${
          est.categoria || ""
        }" hora_apertura "${est.hora_apertura || ""}" hora_cierre "${
          est.hora_cierre || ""
        }" dias_laborales "${est.dias_laborales || ""}" telefono "${
          est.telefono || ""
        }" rating "${est.rating || ""}"\n`
      );
    }
    console.log(
      `Cargados ${establecimientos.rows.length} establecimientos en Redis`
    );
    ...
    for (const promo of promociones.rows) {
      ...
      // Guardar comando para archivo
      redisCommands.write(
        `ZADD promociones:activas ${timestampExp} "${promo.id}"\n`
      );
      ...
      await redisClient.hSet(`promocion:${promo.id}`, {
        ...
        fecha_expiracion:
          promo.fecha_expiracion?.toISOString().split("T")[0] || "",
        establecimiento_id: promo.establecimiento_id?.toString() || "",
      });

      // Guardar comando para archivo
      redisCommands.write(
        `HSET promocion:${promo.id} oferta "${promo.oferta}" fecha_inicio "${
          promo.fecha_inicio?.toISOString().split("T")[0] || ""
        }" fecha_expiracion "${
          promo.fecha_expiracion?.toISOString().split("T")[0] || ""
        }" establecimiento_id "${promo.establecimiento_id || ""}"\n`
      );

      // Añadir promoción al conjunto de promociones del establecimiento
      if (promo.establecimiento_id) {
        await redisClient.sAdd(
          `establecimiento:${promo.establecimiento_id}:promociones`,
          promo.id.toString()
        );
        redisCommands.write(
          `SADD establecimiento:${promo.establecimiento_id}:promociones "${promo.id}"\n`
        );
      }
    }
    ...
    for (const visita of visitas.rows) {
      ...
      // Guardar comando para archivo
      redisCommands.write(
        `ZADD establecimientos:por_visitas ${visita.total_visitas} "${visita.establecimiento_id}"\n`
      );
    }
    console.log(
      `Cargados datos de visitas para ${visitas.rows.length} establecimientos en Redis`
    );
    ...
    for (const visita of visitasPorMes.rows) {
      ...
      // Guardar comando para archivo
      redisCommands.write(
        `ZADD establecimientos:por_visitas:${visita.mes} ${visita.total_visitas} "${visita.establecimiento_id}"\n`
      );
    }
    console.log(
      `Cargados datos de visitas mensuales para ${visitasPorMes.rows.length} registros en Redis`
    );
    ...
    for (const platillo of platillosConsumidos.rows) {
      ...
      await redisClient.zAdd(
        `establecimiento:${platillo.establecimiento_id}:platillos_por_consumo`,
        {
      ...
      // Guardar comando para archivo
      redisCommands.write(
        `ZADD establecimiento:${platillo.establecimiento_id}:platillos_por_consumo ${platillo.total_consumos} "${platillo.comida_id}"\n`
      );
      ...
      // Guardar comando para archivo
      redisCommands.write(
        `HSET platillo:${platillo.comida_id} nombre "${
          platillo.nombre
        }" precio "${platillo.precio || ""}" descripcion "${
          platillo.descripcion || ""
        }"\n`
      );
    }
    console.log(
      `Cargados ${platillosConsumidos.rows.length} registros de consumo de platillos en Redis`
    );
